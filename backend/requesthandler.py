...
api_endpoint_params = 'opening=true&evals=true&clocks=true'
def api_getallgamesbyuser(username,token):  
    url = f'https://lichess.org/api/games/user/{username}?'     
    headers = {'Accept': 'application/x-ndjson','Authorization': f'Bearer {token}'}
    response = requests.request("GET", url+api_endpoint_params, headers=headers)
    games= [json.loads(line, object_hook=dict) for line in response.text.splitlines()]
    return games
            
def request_analysis(username,password,gameid):
    url = "https://lichess.org:443/login"
    request_analysis_url = f'https://lichess.org/{gameid}/request-analysis'
    ...
    session.post(request_analysis_url, headers=headers)
